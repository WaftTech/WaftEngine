...
function* loadCategory(action) {
  ...
  yield call(Api.get('faq/cat', actions.loadCategorySuccess, actions.loadCategoryFailure, token));
}
...
function* loadAll(action) {
  ...
  yield call(Api.get(`faq?find_${search}&page=1&size=10&${sort}${pageNumber}`, actions.loadAllSuccess, actions.loadAllFailure, token));
}

function* loadOne(action) {
  const token = yield select(makeSelectToken());
  yield call(Api.get(`faq/${action.payload}`, actions.loadOneSuccess, actions.loadOneFailure, token));
}

function* redirectOnSuccess() {
  yield take(types.ADD_EDIT_SUCCESS);
  yield put(push('/wt/faq-manage'));
}
