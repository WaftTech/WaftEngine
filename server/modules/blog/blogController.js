...
blogcontroller.GetBlogAuthorize = async (req, res, next) => {
  try {
    ...
    selectq =
      "title description summary tags author short_description meta_tag meta-description category keywords slug_url is_published published_on is_active image added_by added_at updated_at updated_by";
    searchq = {
      is_deleted: false
    };
...
blogcontroller.GetBlogUnauthorize = async (req, res, next) => {
  try {
    ...
    selectq =
      "title description summary tags author short_description meta_tag meta-description category keywords slug_url published_on is_active image added_by added_at updated_at updated_by";
    searchq = {
      is_deleted: false
    };
    searchq = {
...
blogcontroller.GetBlogCategory = async (req, res, next) => {
  try {
    ...
    selectq =
      "title slug_url description image is_active added_by added_at is_deleted";
...
blogcontroller.GetBlogByCat = async (req, res, next) => {
  try {
    ...
    const slug = req.params.slug_url;
    const cat = await blogCatSch.findOne(
      { slug_url: slug, is_deleted: false },
      { _id: 1, title: 1 }
    );
    populate = [
      {
...
blogcontroller.GetBlogByTag = async (req, res, next) => {
  try {
    ...
    const tagBlog = await otherHelper.getquerySendResponse(
      blogSch,
      page,
      size,
      "",
...
blogcontroller.GetBlogByAuthor = async (req, res, next) => {
  try {
    ...
    const blogByAuthor = await otherHelper.getquerySendResponse(
      blogSch,
      page,
      size,
      "",
    ...
    return otherHelper.paginationSendResponse(
      ...
      true,
      blogByAuthor.data,
      "blogs by author get success!!",
      page,
      size,
      blogByAuthor.totaldata
    );
  } catch (err) {
    next(err);
  }
