...
roleController.AddRoles = async (req, res, next) => {
  try {
    ...
    if (role._id) {
      ...
      return otherHelper.sendResponse(res, HttpStatus.OK, true, update, null, config.roleSave, null);
    } else {
      role.Added_by = req.user.id;
      const newRole = new RoleSch(role);
      const roles = await newRole.save();
      return otherHelper.sendResponse(res, HttpStatus.OK, true, roles, null, config.roleSave, null);
    }
  } catch (err) {
    next(err);
  }
...
roleController.GetModule = async (req, res, next) => {
  try {
    ...
    return otherHelper.paginationSendResponse(res, HttpStatus.OK, true, datas.data, config.moduleGet, page, size, datas.totaldata);
  } catch (err) {
    next(err);
  }
};
